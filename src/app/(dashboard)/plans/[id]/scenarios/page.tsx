...
const PREDEFINED_SCENARIOS = [
  {
    ...
    name: 'Prudente',
    icon: ShieldAlert,
    color: { bg: 'bg-amber-500/10 dark:bg-amber-500/20', border: 'border-amber-300', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-800', bar: '#f59e0b' },
  ...
  },
  {
    ...
    name: 'Realistico',
    icon: Target,
    color: { bg: 'bg-blue-500/10 dark:bg-blue-500/20', border: 'border-blue-300', text: 'text-blue-700', badge: 'bg-blue-100 text-blue-800', bar: '#3b82f6' },
  ...
  },
  {
    ...
    name: 'Ottimistico',
    icon: Rocket,
    color: { bg: 'bg-emerald-500/10 dark:bg-emerald-500/20', border: 'border-emerald-300', text: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-800', bar: '#10b981' },
    sale_price_variation: 10,
...
const CUSTOM_COLOR = { bg: 'bg-purple-500/10 dark:bg-purple-500/20', border: 'border-purple-300', text: 'text-purple-700', badge: 'bg-purple-100 text-purple-800', bar: '#8b5cf6' };
...
export default function ScenariosPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl space-y-8">
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              ...
              <div className="bg-muted rounded-xl p-4">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Costo Totale</p>
                <p className="text-xl font-bold text-foreground mt-1">{formatCurrency(baseResults.total_cost)}</p>
              </div>
              <div className={cn('rounded-xl p-4', baseResults.gross_margin >= 0 ? 'bg-emerald-500/10 dark:bg-emerald-500/20' : 'bg-red-500/10 dark:bg-red-500/20')}>
              ...
              <div className={cn('rounded-xl p-4', baseResults.roi >= 0 ? 'bg-emerald-500/10 dark:bg-emerald-500/20' : 'bg-red-500/10 dark:bg-red-500/20')}>
      ...
      {/* ============================================================ */}
      <div>
        <h2 className="text-lg font-semibold text-foreground mb-4">Scenari Predefiniti</h2>
              ...
              <Card key={scenario.key} className={cn('border-2 overflow-hidden', scenario.color.border)}>
                ...
                <CardContent className="pt-4 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Ricavo</span>
                    <span className="text-sm font-semibold text-foreground">{formatCurrency(r.total_revenue)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Costo Totale</span>
                    <span className="text-sm font-semibold text-foreground">{formatCurrency(r.total_cost)}</span>
                  </div>
                  <div className="border-t border-border pt-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-foreground">Margine</span>
                  ...
                  {scenario.key !== 'realistic' && (
                    <div className={cn(
                      ...
                      deltaMargin > 0 ? 'bg-emerald-500/10 dark:bg-emerald-500/20' : deltaMargin < 0 ? 'bg-red-500/10 dark:bg-red-500/20' : 'bg-muted'
                    )}>
                      <span className="text-xs text-muted-foreground block">Differenza dal base</span>
                      <p className={cn(
                        'text-sm font-bold mt-0.5',
                        deltaMargin > 0 ? 'text-emerald-600' : deltaMargin < 0 ? 'text-red-600' : 'text-muted-foreground'
                      )}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-semibold text-foreground">Prezzo di Vendita</label>
                    <span className={cn(
                      'text-sm font-bold px-2 py-0.5 rounded',
                      customSaleVariation > 0 ? 'bg-emerald-100 text-emerald-700' :
                      customSaleVariation < 0 ? 'bg-red-100 text-red-700' : 'bg-muted text-foreground'
                    )}>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-semibold text-foreground">Costo Lavori</label>
                    <span className={cn(
                      'text-sm font-bold px-2 py-0.5 rounded',
                      customConstructionVariation < 0 ? 'bg-emerald-100 text-emerald-700' :
                      customConstructionVariation > 0 ? 'bg-red-100 text-red-700' : 'bg-muted text-foreground'
                    )}>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-semibold text-foreground">Costo Acquisizione</label>
                    <span className={cn(
                      'text-sm font-bold px-2 py-0.5 rounded',
                      customAcquisitionVariation < 0 ? 'bg-emerald-100 text-emerald-700' :
                      customAcquisitionVariation > 0 ? 'bg-red-100 text-red-700' : 'bg-muted text-foreground'
                    )}>
              ...
              {customResults && (
                <div className={cn('rounded-xl p-6', CUSTOM_COLOR.bg)}>
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <h3 className="font-semibold text-foreground">Risultati Simulazione</h3>
                  </div>

                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">Ricavo</span>
                      <span className="text-lg font-semibold text-foreground">{formatCurrency(customResults.total_revenue)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">Costo Totale</span>
                      <span className="text-lg font-semibold text-foreground">{formatCurrency(customResults.total_cost)}</span>
                    </div>
                    ...
                    <div className={cn(
                      ...
                      customResults.gross_margin - baseResults.gross_margin < 0 ? 'bg-red-100' : 'bg-muted'
                    )}>
                      <span className="text-xs text-muted-foreground block">Differenza dal piano base</span>
                      <p className={cn(
                        'text-sm font-bold mt-0.5',
                        customResults.gross_margin - baseResults.gross_margin > 0 ? 'text-emerald-700' :
                        customResults.gross_margin - baseResults.gross_margin < 0 ? 'text-red-700' : 'text-foreground'
                      )}>
      ...
      {/* ============================================================ */}
      <div>
        <h2 className="text-lg font-semibold text-foreground mb-4">Confronto Scenari</h2>
            ...
            <Card className="border-2 border-orange-200">
              <CardContent className="p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-foreground text-sm">Break-even Price</h3>
                    <p className="text-xs text-muted-foreground">Ricavo minimo per non andare in perdita</p>
                  </div>
                </div>
            ...
            <Card className="border-2 border-emerald-200">
              <CardContent className="p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-foreground text-sm">Margine di Sicurezza</h3>
                    <p className="text-xs text-muted-foreground">Quanto il prezzo puo scendere</p>
                  </div>
                </div>
                <p className={cn(
                  'text-2xl font-bold',
                  ...
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      className={cn(
                        ...
                        breakEvenAnalysis.safetyMarginPercent > 10 ? 'bg-emerald-500/10 dark:bg-emerald-500/200' :
                        breakEvenAnalysis.safetyMarginPercent > 5 ? 'bg-amber-500/10 dark:bg-amber-500/200' : 'bg-red-500/10 dark:bg-red-500/200'
                      )}
            ...
            <Card className={cn(
              ...
              <CardContent className="p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-foreground text-sm">Valutazione Operazione</h3>
                    <p className="text-xs text-muted-foreground">Giudizio complessivo</p>
                  </div>
                </div>
                <p className={cn(
                  'text-lg font-bold',
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-3 px-4 font-medium text-muted-foreground">Indicatore</th>
                      {allScenarios.map((s) => (
                        <th key={s.key} className="text-right py-3 px-4 font-medium text-muted-foreground">
                          {s.name}
                        </th>
                      ))}
                    </tr>
                    ...
                    {[
                      ...
                      <tr
                        ...
                        className={cn(
                          ...
                          row.highlight && 'bg-muted font-semibold'
                        )}
                      >
                        <td className="py-3 px-4 font-medium text-foreground">{row.label}</td>
                        {allScenarios.map((s) => (
                          <td key={s.key} className="py-3 px-4 text-right text-foreground">
