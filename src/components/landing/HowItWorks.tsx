...
export function HowItWorks() {
  return (
    <section id="how-it-works" className="py-28 bg-background/30 backdrop-blur-[2px]">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((item, idx) => (
            <div key={idx} className="relative text-center group">
              ...
              <div className="relative z-10 inline-flex items-center justify-center w-24 h-24 rounded-2xl bg-primary/5 dark:bg-primary/10 border border-primary/20 mb-7 group-hover:scale-105 group-hover:shadow-[0_8px_24px_hsl(var(--primary)/0.12)] transition-all duration-300">
                <item.icon className="w-10 h-10 text-primary" />
                <span className="absolute -top-2 -right-2 w-8 h-8 icon-gradient rounded-xl flex items-center justify-center text-white text-xs font-bold shadow-[0_2px_8px_hsl(var(--primary)/0.4)]">
                  {item.step}
                </span>
              </div>
